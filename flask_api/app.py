...
@app.route("/api/recommend/categories/<user_id>", methods=["GET"])
def recommend_categories(user_id):
    """Retourne les recommandations par catégorie"""
    if rec_system is None:
        return jsonify({"error": "Le système de recommandation n'est pas initialisé."}), 500
    try:
        cats = rec_system.get_recommendations_by_category(user_id, n_per_category=5)
        out = {k: v.to_dict(orient="records") if isinstance(v, pd.DataFrame) else v for k, v in cats.items()}
        return jsonify(out)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
